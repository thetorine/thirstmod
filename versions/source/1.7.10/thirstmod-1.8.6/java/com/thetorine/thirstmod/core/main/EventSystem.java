...
import com.thetorine.thirstmod.core.client.ClientProxy;
...
public class EventSystem implements IGuiHandler {
	...
	public void onHarvest(BlockEvent.HarvestDropsEvent event) {
		if(event.harvester != null) {
			PlayerContainer player = PlayerContainer.getPlayer(event.harvester.getDisplayName());
			if(FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
	...
	public void onSleep(PlayerSleepInBedEvent event) {
		if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
			int sleepingTime = (int) (24000 - event.entityPlayer.worldObj.getWorldTime());
			int interval = 2000;
			int thirstLoss = -Math.round((float)sleepingTime / (float)interval);
			
			PlayerContainer player = PlayerContainer.getPlayer(event.entityPlayer.getDisplayName());
			if ((player.stats.thirstLevel - thirstLoss) <= 8) {
				player.player.addChatMessage(new ChatComponentText("You are too thirsty to sleep!"));
				event.result = EntityPlayer.EnumStatus.OTHER_PROBLEM;
			} else {
				player.stats.addStats(thirstLoss, 0f);
			}
		}
	}
