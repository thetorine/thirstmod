...
public class BlockDrinkShop extends BlockContainer {

	private IIcon frontSide;
	private IIcon remainSide;

	public BlockDrinkShop() {
		super(Material.rock);
		setResistance(5f);
		setHardness(4f);
	...
	public boolean onBlockActivated(World par1World, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
		player.openGui(ThirstMod.INSTANCE, 93, par1World, x, y, z);
		return true;
	}
	...
	public void registerBlockIcons(IIconRegister icon) {
		...
		remainSide = icon.registerIcon("thirstmod:ds_remain");
	}

	@SideOnly(Side.CLIENT)
	@Override
	public IIcon getIcon(int side, int metadata) {
		return side != metadata ? remainSide : frontSide;
	}
}
